...
-- ============================================================
-- v12 additions — item availability toggle, add-on orders, and the
-- two-player "Goal Rush" waiting-room game. See
-- migration_v12_availability_addon_game.sql for notes; kept in sync
-- here so a brand-new install gets these without running migrations.
-- ============================================================
...
-- ---------- Edit an order's items (any signed-in staff) ----------
-- Replaces the full item list of an open/preparing/ready order — used
-- when staff need to fix a mistake (wrong item, wrong quantity) rather
-- than just adding more on top. Recalculates totals from scratch.
...
-- ---------- Void an order (cancel it, keep the record for the audit trail) ----------
-- Never deletes the row — sets status to 'void' so the order still
-- shows up in history/reporting as "this was cancelled," not silently
-- erased. The app restricts which roles can trigger this (matching
-- the same pattern used for closing/payment) — see pos.html.
